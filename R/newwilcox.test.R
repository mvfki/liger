...
newwilcox.test <- function(x, ...) UseMethod("newwilcox.test")

newwilcox.test.default <-
                ...
                switch(alternative,
                       "two.sided" = {
                           p <- if(STATISTIC > (n.x * n.y / 2))
                               wrap_pwilcox(STATISTIC - 1, n.x, n.y, lower.tail = FALSE)
                           else
                               wrap_pwilcox(STATISTIC, n.x, n.y)
                           min(2 * p, 1)
                       },
                       "greater" = {
                           wrap_pwilcox(STATISTIC - 1, n.x, n.y, lower.tail = FALSE)
                       },
                       "less" = wrap_pwilcox(STATISTIC, n.x, n.y))
            if(conf.int) {
                ...
                cint <-
                    switch(alternative,
                           "two.sided" = {
                               qu <- wrap_qwilcox(alpha/2, n.x, n.y)
                               if(qu == 0) qu <- 1
                               ql <- n.x*n.y - qu
                               achieved.alpha <- 2*wrap_pwilcox(trunc(qu)-1,n.x,n.y)
                               c(diffs[qu], diffs[ql + 1])
                           },
                           "greater" = {
                               qu <- wrap_qwilcox(alpha, n.x, n.y)
                               if(qu == 0) qu <- 1
                               achieved.alpha <- wrap_pwilcox(trunc(qu)-1,n.x,n.y)
                               c(diffs[qu], +Inf)
                           },
                           "less" = {
                               qu <- wrap_qwilcox(alpha, n.x, n.y)
	    ...
	    PVAL <- switch(alternative,
			...
			if(correct) {
                ...
                root <- function(zq) {
                    ...
                    ## e.g.  newwilcox.test(1, 2:60, conf.int=TRUE)
...
function(formula, data, subset, na.action, ...)
{
    ...
    DATA <- setNames(split(mf[[response]], g),
		     c("x", "y"))
    y <- do.call("newwilcox.test", c(DATA, list(...)))
    y$data.name <- DNAME
    y
}
